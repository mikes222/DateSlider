...
public class ScrollLayout extends LinearLayout {
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            TimeView ttv = mLabeler.createView(context, i == centerIndex);
            addView((View) ttv, lp);
        }
        ...
        for (int i = centerIndex + 1; i < childCount; i++) {
            ...
            if (thisView != null && lastView != null) {
                thisView.setTime(mLabeler.add(lastView.getEndTime(), 1));
            }
        }
    ...
    private void setTime(long time, int loops) {
        ...
        TimeView timeView = (TimeView) getChildAt(pos);
        if (timeView == null) {
            return;
        }
        if (loops > 2 || timeView.getStartTime() <= time && timeView.getEndTime() >= time) {
            if (loops > 2) {
                Log.d(TAG, String.format("time: %d, start: %d, end: %d", time, timeView.getStartTime(), timeView.getEndTime()));
                return;
            }
            double center = getWidth() / 2.0;
            int left = (getChildCount() / 2) * objWidth - getScrollX();
            double curr_per = (center - left) / objWidth;
            double goal_per = (time - timeView.getStartTime()) / (double) (timeView.getEndTime() - timeView.getStartTime());
            int shift = (int) Math.round((curr_per - goal_per) * objWidth);
            mScrollX -= shift;
            reScrollTo(mScrollX, 0, false);
        } else {
            double diff = timeView.getEndTime() - timeView.getStartTime();
            int steps = (int) Math.round(((time - (timeView.getStartTime() + diff / 2)) / diff));
            moveElements(-steps);
    ...
    protected void moveElements(int steps) {
        ...
        for (int i = start; i != end; i += direction) {
            ...
            if (index >= 0 && index < getChildCount()) {
                tv.setTime((TimeView) getChildAt(index));
            } else {
                tv.setTime(mLabeler.add(tv.getEndTime(), -steps));
            }
